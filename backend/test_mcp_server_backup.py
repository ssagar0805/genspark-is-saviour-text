...
class MCPServerTester:
    ...
    async def test_web_search(self):
        ...
        try:
            # Test web search
            result = await search_tool.custom_search("artificial intelligence news", 3)
            ...
            if result["success"] and len(result.get("results", [])) > 0:
                ...
                self.log_result(
                    test_name, True,
                    f"Found {num_results} results, first: '{first_title[:50]}...'",
                    time.time() - start_time
                )
                return True
            else:
                self.log_result(
                    test_name, False,
                    f"Search failed: {result.get('error', 'No results found')}",
                    time.time() - start_time
                )
                return False
    ...
    async def test_fact_check(self):
        ...
        try:
            # Test fact checking
            result = await search_tool.factcheck_search("COVID-19 vaccine effectiveness", 2)
            
            if result["success"]:
                num_checks = len(result.get("fact_checks", []))
                self.log_result(
                    test_name, True,
                    f"Found {num_checks} fact-check results",
                    time.time() - start_time
                )
                return True
            else:
                self.log_result(
                    test_name, False,
                    f"Fact check failed: {result.get('error', 'No fact checks found')}",
                    time.time() - start_time
                )
                return False
    ...
    async def test_comprehensive_search(self):
        ...
        try:
            ...
            result = await search_tool.comprehensive_search("climate change facts")
            ...
            if result["success"]:
                ...
                self.log_result(
                    test_name, False,
                    f"Comprehensive search failed: {result.get('error', 'Unknown error')}",
                    time.time() - start_time
                )
                return False
